---
name: re-bypass-themida
version: 0.3.0
status: implemented
family: anti-tamper-vendors
severity: critical
catalog_entry: anti-tamper-vendors.themida
...
**v0.3.0 (v0.8.0+ Wave 2 Item F): Themida / WinLicense unpacking via anpa1200/Unpacker.** Replaces the v0.2.0 reference to the unmaintained Themida-Unpacker (which 404'd in 2024-Q2).

## When to use this skill

The catalog's anti-tamper-vendors.themida entry matches. The skill delegates to [anpa1200/Unpacker](https://github.com/anpa1200/Unpacker) (vendored at `vendored/anpa1200-Unpacker/`), which integrates:
- Unipacker (32-bit, for legacy Themida 2.x)
- Qiling (64-bit, for modern Themida 3.x)

See `vendored/anpa1200-Unpacker/README.md` for the one-time setup.

## Tools invoked

- `mcp__re_vendor_anti_tamper.run_vendor_tool(vendor="themida", target=...)` — actually invokes anpa1200 (v0.8.0+)
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `anti-tamper-vendors.themida` with high confidence.
...
- [Catalog entry this skill implements](../../data/catalog.json) — `anti-tamper-vendors.themida`
