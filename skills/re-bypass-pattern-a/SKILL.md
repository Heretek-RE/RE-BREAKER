---
name: re-bypass-pattern-a
version: 0.2.0
status: implemented
family: encrypted-vm-bytecode-interpreter
severity: critical
catalog_entry: encrypted-vm.bytecode-interpreter.pattern-a
playbook: docs/PLAYBOOKS/encrypted-vm-bytecode-interpreter-pattern-a.md
pattern_yaml: data/patterns/pattern-a.yml
---

# re-bypass-pattern-a

**v0.2.0 implemented.** Pattern A: encrypted-VM bytecode interpreter (Unity IL2CPP target)

## When to use this skill

re-catalog-match returns encrypted-vm.bytecode-interpreter.pattern-a (high confidence) for the target. Section set includes .xtls/.xpdata/.xdata/.arch/.link/.sbss/.xcode and re-anti-analysis reports ≥50 RDTSC sites.
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `encrypted-vm.bytecode-interpreter.pattern-a` with high confidence.
...
- **RE-UNLEASHED doc**: `protection/encrypted-vm-bytecode-interpreter/pattern-a-il2cpp.md` — Internal/vendor-named deep-dive.
...
- [Catalog entry this skill implements](../../data/catalog.json) — `encrypted-vm.bytecode-interpreter.pattern-a`
- [Pattern YAML](../../data/patterns/pattern-a.yml)
- [Playbook](../../docs/PLAYBOOKS/encrypted-vm-bytecode-interpreter-pattern-a.md)
