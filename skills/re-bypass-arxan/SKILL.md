---
name: re-bypass-arxan
version: 0.2.0
status: implemented
family: anti-tamper-vendors
severity: high
catalog_entry: anti-tamper-vendors.arxan
...
**v0.2.0 implemented.** Arxan / Digital.ai (code-protection + app-shielding)

## When to use this skill

The catalog's anti-tamper-vendors.arxan entry matches. NO open-source tool exists. Per MRTEA SOW-X §E.3, white-box keys are not extracted as a deliverable.
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `anti-tamper-vendors.arxan` with high confidence.
...
- [Catalog entry this skill implements](../../data/catalog.json) — `anti-tamper-vendors.arxan`
