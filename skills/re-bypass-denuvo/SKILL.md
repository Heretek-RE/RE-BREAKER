...
**v0.2.0 implemented.** Denuvo Anti-Tamper (Denuvo Anti-Cheat is a separate product)

## When to use this skill

The catalog's anti-tamper-vendors.denuvo entry returns a match. There is no general Denuvo bypass tool; per-title bypass requires months of work. This skill is a placeholder that links to the A-DW + POGO approach.
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `anti-tamper-vendors.denuvo` with high confidence.
...
- **RE-UNLEASHED doc**: `publishers/atlus-sega/persona-3-reload/` — The P3R per-game doc (the canonical Denuvo ATD case).
...
- [Catalog entry this skill implements](../../data/catalog.json) — `anti-tamper-vendors.denuvo`
